...
Office files are zip archives. This script:
  ...
  2. embeds every PNG from ../../shared/icons/ as customUI/images/<name>.png
...
def main() -> None:
    ...
    ribbon_xml = app_dir / "ribbon" / "customUI14.xml"
    images_dir = repo_dir / "shared" / "icons"
    ...
    if refs - have:
        sys.exit(f"Missing icon files for: {', '.join(sorted(refs - have))} "
                 f"- run ../../scripts/make_icons.py first")
