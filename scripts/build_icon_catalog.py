...
def outputs() -> dict[Path, str]:
    ...
    The icon geometry used to be emitted as per-icon VBA modules
    (modIconAid.bas / modIconAidData*.bas). Those were retired when the add-in
    moved to the file-backed loader in modIconAidCurated.bas, which reads the
    external icons.dat instead - far more icons than a Mac PowerPoint project can
    hold. Nothing references the old modules, so they are no longer generated.
    """
    validate_catalog(ACTIVE_ICONS)
    return {
